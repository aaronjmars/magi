...
const html = (url, imgurUrl) => {
  const warpcastComposeUrl = `https://warpcast.com/~/compose?embeds[]=https://magi.lol/degen&embeds[]=${encodeURIComponent(imgurUrl)}`;

  return `
  <!DOCTYPE html>
  <html lang="en">
    <head>
    <title>Magi - Join the Degen meme revolution.</title>
      <meta property="og:title" content="Magi - Join the Degen meme revolution.">
      <meta property="og:image" content="${url}">
      <meta name="fc:frame" content="vNext">
      <meta name="fc:frame:image" content="${url}">

      <meta property="fc:frame:post_url" content="https://magi.lol/api/frame-degen" />

      <meta property="fc:frame:button:1" content="Explore Magi" />
      <meta property="fc:frame:button:1:action" content="link" />
      <meta property="fc:frame:button:1:target" content="https://magi.lol/degen" />
...
async function getImage() {
  const randomPage = Math.floor(Math.random() * 36) + 1;
...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    await track("Degen meme fetched");
